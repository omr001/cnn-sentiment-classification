...
class CNN_max(Chain):
    def __init__(self, vocab_size, embedding_size, input_channel, output_channel_1, output_channel_2, output_channel_3, k1size, k2size, k3size, pooling_units, output_size=5, train=True):
    ...
    def __call__(self, x):
        ...
        h1 = self.max_pooling(F.relu(c1), b, outputC, fixed_len, tf)
        ...
        tf = self.set_tfs(ox, b, outputC, fixed_len)  # true&flase
        h2 = self.max_pooling(F.relu(c2), b, outputC, fixed_len, tf)
        ...
        tf = self.set_tfs(ox, b, outputC, fixed_len)  # true&flase
        h3 = self.max_pooling(F.relu(c3), b, outputC, fixed_len, tf)
    ...
    def max_pooling(self, c, b, outputC, fixed_len, tf):
        masked_c = F.where(tf, c, Variable(-1000*xp.ones((b, outputC, fixed_len, 1)).astype(xp.float32), volatile='auto'))
        p = F.max_pooling_2d(masked_c, fixed_len)
        return p
...
if __name__ == "__main__":
    ...
    k3 = 5
    p_units = 300
    cnn = CNN_max(vocab_size, emb_size, ic, oc1, oc2, oc3, k1, k2, k3, p_units)
